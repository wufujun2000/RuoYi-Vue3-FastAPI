...
| 中间件 | 作用 |
| --- | --- |
| `context_middleware` | 清理请求级上下文，避免脏状态泄漏 |
| `cors_middleware` | 控制跨域访问 |
| `gzip_middleware` | 压缩响应 |
| `api_response_header_middleware` | 增补统一响应头 |
| `trace_middleware` | 记录请求 Trace 上下文 |
| `demo_mode_middleware` | 演示环境限制危险操作 |
| `transport_crypto_middleware` | 处理前后端传输层加解密 |

## 7. 异常处理体系

`exceptions/handle.py` 统一把异常映射为标准响应：

- `AuthException`：认证失败，返回未授权响应
- `LoginException`：登录失败，返回业务失败响应
- `PermissionException`：权限不足，返回禁止访问响应
- `ServiceException`：服务异常，返回错误响应
- `ServiceWarning`：业务警告，返回失败响应
- `FieldValidationError`：字段校验异常
- `HTTPException`：HTTP 级异常
- `Exception`：兜底异常
